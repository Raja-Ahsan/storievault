...
const Create = ({ blogCategories = [], blogTags = [], flash }) => {
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route('admin-dashboard.blog-posts.store'), {
      forceFormData: true,
      preserveScroll: true,
      onError: () => {
              ...
              <CForm onSubmit={submit}>
                  ...
                  <CCardBody>
                    <CRow className="g-3">
                      ...
                      <CCol md={6}>
                        ...
                        <CFormInput value={data.meta_tags} onChange={(e) => setData('meta_tags', e.target.value)} placeholder="comma-separated" />
                      </CCol>
                      <CCol md={12}>
                    ...
                    {(data.faqs || []).map((faq, index) => (
                      <CCard key={index} className="mb-3">
                        <CCardBody className="pb-2">
                          ...
                          <CFormLabel htmlFor={`blog-faq-q-${index}`}>Question</CFormLabel>
                          <CFormInput
                            id={`blog-faq-q-${index}`}
                            value={faq.question || ''}
                            ...
                            className="mb-2"
                          />
                          <CFormLabel htmlFor={`blog-faq-a-${index}`}>Answer</CFormLabel>
                          <CFormTextarea
                            id={`blog-faq-a-${index}`}
                            value={faq.answer || ''}
                ...
                <CRow className="mb-4">
                  <CCol md={6}>
                    ...
                    {preview && (
                      <img src={preview} alt="" className="mt-2 rounded" style={{ maxWidth: 240, maxHeight: 160, objectFit: 'cover' }} />
                    )}
                  </CCol>
                </CRow>
